...
/**
 * 소셜 가입 사용자는 profiles.dong 이 null 이다 (#83) — 첫 로그인 때 동네 인증을 유도한다.
    ...
    <div className="mx-6 mt-4 flex items-center gap-2.5 rounded-[12px] border-[1.5px] border-dashed border-[#9fd4ae] bg-[#f5faf4] px-4 py-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        <div className="text-xs text-[#6b8573]">
          {dongBusy ? "접속 위치를 확인하는 중…" : dongDetail || "접속 위치(IP)로 우리 동네를 확인해요"}
        </div>
      </div>
      {dongDetail && !dongBusy ? (
        <>
          <input
            ...
            className="w-[160px] rounded-[9px] border-[1.5px] border-[#d5e6d6] bg-white px-3 py-[7px] text-[13px] outline-none focus:border-[#1f8a4c]"
          />
          <div
            onClick={confirmDong}
