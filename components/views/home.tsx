...
export default function HomeView() {
  ...
  /**
   * 목록에서 빠진 카드는 떨어지는 애니메이션을 끝내고 버린다 (#174, #181).
   *
   * 판정 기준은 원본(deals)이 아니라 **화면 목록(cards)** 이다 — 주최자 취소는 행을 지우지 않고
   * status 만 canceled 로 바꾸므로, 원본만 보면 취소된 카드가 아무 연출 없이 사라진다.
   * 대신 필터·검색이 바뀐 렌더에서는 건너뛴다 (guard) — 그건 데이터가 사라진 게 아니다.
   */
  const display = useLeaving(cards, `${filter}|${statusFilter}|${myDealsOnly}|${search}`);
