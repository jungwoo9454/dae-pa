      ...
      <div className="mx-auto flex w-[960px] justify-center gap-[30px]">
        <div className="w-[420px] flex-none">
          {topupResult && (
            <div
              ...
              className={`mb-3.5 cursor-pointer border-[1.5px] px-3.5 py-2.5 text-[13px] font-bold ${
                topupResult === "ok"
          ...
          <div className="rounded-md bg-[#26262b] p-6 shadow-[0_14px_28px_rgba(27,25,23,.3)]">
            <div className="text-[11px] tracking-[.2em] text-[#9b948c]">대파페이 · 금고 잔액</div>
            <div className="mt-3 rounded border border-[#3c3c42] bg-[#111114] px-5 py-[18px] text-right">
              <span
                className="tnum text-[40px] font-bold text-[#7ce28c]"
                style={{ textShadow: "0 0 12px rgba(124,226,140,.5)" }}
              >
                {balance.toLocaleString("ko-KR")}
              </span>
              <span className="text-[15px] text-[#7ce28c] opacity-70"> 원</span>
            </div>
            ...
            <div className="mt-[18px] flex items-center border-t border-dashed border-[#3c3c42] pt-4">
              <div>
                <div className="text-[13px] font-bold text-[#e4e4e0]">정산 자동 결제</div>
                <div className="mt-1 text-[10.5px] text-[#9b948c]">정산 확정 시 잔액에서 즉시 차감</div>
              </div>
          ...
          {topupOpen && (
            <div className="receipt mt-4 p-[18px]">
              ...
              />
              {topupAmt <= 0 ? (
                <div className="key key-off mt-2.5 py-2.5 text-[13px]">[ 금액을 입력해주세요 ]</div>
              ) : (
                <div onClick={doTopup} className="key key-primary mt-2.5 py-2.5 text-[13px]">
                  [ {fmt(topupAmt)} 충전하기 ]
                </div>
              )}
              <div className="mt-2.5 text-[11px] text-[#9c9ca3]">
          ...
          {withdrawOpen && (
            <div className="receipt mt-4 p-[18px]">
              ...
              />
              {withdrawAmt > balance ? (
                <div className="key key-off mt-2.5 py-2.5 text-[13px]">[ 잔액이 부족해요 ]</div>
              ) : (
                <div onClick={doWithdraw} className="key key-primary mt-2.5 py-2.5 text-[13px]">
        ...
        <div className="w-[500px] flex-none">
          <div className="receipt px-7 py-6">
            ...
            {history.length === 0 && (
              <div className="py-10 text-center text-[13px] text-[#8b8478]">이용 내역이 없어요</div>
            )}
            {history.map((h, i) => (
              <div key={i} className="rule-dot flex items-center gap-3 py-[15px] text-[13px]">
                <span
                  className={`flex-none px-2.5 py-[3px] text-[11px] font-bold ${
                    h.amt > 0
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-[3px] text-[10.5px] text-[#9c9ca3]">{h.when}</div>
                </div>
