...
export interface StatusView {
  key: "settle" | "closed" | "closing" | "recruit";
  label: string;
  bg: string;
  fg: string;
}
...
export function statusOf(d: Deal, now: number): StatusView {
  if (d.status === "settle") return { key: "settle", label: "정산중", bg: "#e0f0f1", fg: "#0e7490" };
  ...
  return { key: "recruit", label: "모집중", bg: "#e9f6ec", fg: "#166b3a" };
}

export function remainLabel(d: Deal, now: number) {
  if (d.status === "settle") return "마감됨";
  const ms = d.end - now;
  if (ms <= 0) return "마감됨";
...
export function joinable(d: Deal, now: number) {
  return !d.me && d.status !== "settle" && d.end - now > 0;
}

export function joinLabel(d: Deal, now: number) {
  if (d.status === "settle") return "정산 보기";
  if (d.me) return "참여중 ✓";
